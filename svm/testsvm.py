...
def scale(dataSet): #feature scaling
	for j in range(len(dataSet[0])):
		maxValue = 0
		for i in range(len(dataSet)):
			if abs(dataSet[i][j]) > maxValue:
...
trainDataSet = scale(trainDataSet) #1:feature Scaling
testDataSet = scale(testDataSet)


trainDataSet = mat(trainDataSet) # n * n
trainLabelSet = mat(trainLabelSet).T #transform to n * 1
