...
# 采用TF-IDF 算法对选取得到的特征进行计算权重 词频越高文档的TF-IDF值就很可能高
        ...
        for eachclass in dic:        
            ...
            for words in docList:        #期中一个file
                if feature in words:  #如果当前特征在这个file里面，docfeature ++ feature出现频率
                    featureDocNum += 1
        # 计算特征的逆文档频率
        featurevalue = math.log(float(docNum)/(featureDocNum+1)) #计算idf = log（词料库的文档总数/包含该词的文档数+1）
        # 写入文件，特征的文档频率
        ...
        for doc in dic[eachclass]:
            ...
            for feature in features:
                ...
                if feature in doc:
                    ...
                    tf = float(featureNum)/(len(doc)) #计算 tf = 某个词在文章中出现的总次数/文章的总词数
                    # 计算逆文档频率 tf * idf
