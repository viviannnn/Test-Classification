...
#        print sigma
        ...
        for i in xrange(numSamples):  
            ...
            kernelValue[i] = exp(diff * diff.T / (-2.0 * sigma**2))   #高斯kernel
    else:  
...
def calcKernelMatrix(train_x, kernelOption):
    numSamples = train_x.shape[0]  #number of train sample
    kernelMatrix = mat(zeros((numSamples, numSamples)))  #number * number  matrix
    for i in xrange(numSamples):  
        kernelMatrix[:, i] = calcKernelValue(train_x, train_x[i, :], kernelOption)  #smaple train_x[i, :]
    return kernelMatrix  
...
class SVMStruct:
    def __init__(self, dataSet, labels, C, toler, kernelOption):  
        ...
        self.kernelMat = calcKernelMatrix(self.train_x, self.kernelOpt)  #第一步： 计算核函数maxtrix
...
#SMO算法的工作原理：在如图25所示的最优化问题中，每次循环中选择两个α进行优化处理。一旦找到一对合适的α，那么就增大其中一个同时减小另一个。这里所谓的‘合适’就是指两个α必须要满足一定的条件，条件之一是这两个α必须要在间隔边界之外，而其第二个条件则是这两个α还没有进行过区间化处理或者不在边界上。比如不是一般性的假设先优化的是变量α1和α2，则其他变量α_i(i=3,4,...N)则是固定的。


# the inner loop for optimizing alpha i and alpha j  
def innerLoop(svm, alpha_i):
    error_i = calcError(svm, alpha_i) #第1 计算Ei
...
def trainSVM(train_x, train_y, C, toler, maxIter, kernelOption = ('rbf', 1.0)):  
    ...
    svm = SVMStruct(mat(train_x), mat(train_y), C, toler, kernelOption) # 第一步：计算核函数（高斯）
    # start training
    entireSet = True  
    alphaPairsChanged = 0  #alphapair 变化数初值为0
    iterCount = 0  #最大循环次数
    ...
    while (iterCount < maxIter) and ((alphaPairsChanged > 0) or entireSet):
        alphaPairsChanged = 0 #每次循环 alphapair 变化数初值为0
        # printout = nonzero(svm.alphas.A > 0)[0]
        # for i in printout:
        #     print svm.alphas[i, 0]
        #print printout
        # update alphas over all training examples  
        if entireSet:
#            print svm.numSamples
